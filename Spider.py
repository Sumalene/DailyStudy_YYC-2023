...
def get_edgedriver():
    ...
    # 可选：如果你想让 Edge 无界面运行，取消下面一行的注释
    # edge_options.add_argument('--headless')

    # edge_driver_path 是你下载的 Microsoft Edge WebDriver 的路径
    edge_driver_path = "D:\py310\msedgedriver.exe"  # 根据你的实际路径填写
    # 创建 Microsoft Edge 浏览器驱动对象，用于后续操作浏览器
...
def get_range_url():
    ...
    for i in range(0,10):
        ...
        range_url='https://sz.lianjia.com'+a_href
...
def get_range_price_page_url():
    ...
    for key in range_price_urls:
        ...
        for part_range_price_url in part_range_price_urls:
            ...
            remainder = int(house_count[0]) % 30 #更好的命名是remainder,释义为余数
            #page_count = 0
            if remainder == 0:
